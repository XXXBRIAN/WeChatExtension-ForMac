...
{
    NSMenuItem *quitMonitorItem = [NSMenuItem menuItemWithTitle:YMLanguage(@"退群监控(占用资源过多建议关闭)", @"Group-Quitting Monitor")
                                                         action:@selector(onQuitMonitorItem:)
                                                         target:self
                                                  keyEquivalent:@""
                                                          state:[YMWeChatPluginConfig sharedConfig].quitMonitorEnable];
    NSMenuItem *ZGMPItem = [NSMenuItem menuItemWithTitle:YMLanguage(@"群员行为统计", @"ZGMP")
...
- (void)onUninstallPluginControl:(NSMenuItem *)item
{
    NSAlert *alert = [NSAlert alertWithMessageText:YMLanguage(@"警告", @"WARNING")
                                     defaultButton:YMLanguage(@"取消", @"cancel")                       alternateButton:YMLanguage(@"卸载",@"uninstall")
                                       otherButton:nil
                         informativeTextWithFormat:@"%@", YMLanguage(@"是否卸载Brian小助手？重启生效。", @"Do you want to uninstall the assistant? Restart takes effect.")];
    NSUInteger action = [alert runModal];
